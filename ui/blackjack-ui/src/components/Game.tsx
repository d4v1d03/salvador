...
import {
  gameofcards_CONTRACT_ABI,
  gameofcards_CONTRACT_ADDRESS,
...
export const Game: React.FC<IProps> = ({ library, account }) => {
  ...
  const withdrawBet = async () => {
    try {
      ...
      const gameofcardsContract = new Contract(
        gameofcards_CONTRACT_ADDRESS,
        gameofcards_CONTRACT_ABI,
        signer
      )
      if (score! > 0) {
        const tx: TransactionResponse = await toast.promise(
          gameofcardsContract.withdrawBet(ethers.utils.parseEther("0.02")),

          {
            pending: "Withdrawing...",
            success: "Withdrawn succesfully",
        ...
        const tx: TransactionResponse = await toast.promise(
          gameofcardsContract.withdrawBet(ethers.utils.parseEther("0.01")),
  ...
  const unlockBet = async () => {
    try {
      ...
      if (score! > 0) {
        const data = {
          from: signerAddress,
          to: gameofcards_CONTRACT_ADDRESS,
        ...
        const endGame: TransactionResponse = await gameofcardsContract.endGame(
          account,
